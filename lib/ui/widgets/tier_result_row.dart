...
class TierResultRow extends StatelessWidget {
  const TierResultRow(this.pick, {super.key});
    ...
    return Container(
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: [
            // Left tier-colored accent bar — the RISKY variant is a wider,
            // striped decorative accent (tierRiskyAccent, decorative ONLY —
            // never behind text).
            _AccentBar(color: tierColor, isRisky: isRisky, accent: colors.tierRiskyAccent),
            Expanded(
              child: Padding(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text(
                          // offense is exactly 2 or 4 (SE gate), so .toInt() is
                          // safe and yields "2×"/"4×".
                          '${pick.offense.toInt()}×',
...
/// The left-edge tier accent. Standard rows get an 8px solid bar; the RISKY row
/// gets a wider striped bar in the decorative `tierRiskyAccent` (AC#5) — kept
/// off any text.
class _AccentBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (!isRisky) return Container(width: 8, color: color);
